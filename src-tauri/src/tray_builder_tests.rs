...
mod tray_builder_tests {
    ...
    use crate::{
        tray::{TrayManager, calculate_show_menu_on_left_click, get_tray_tooltip},
        config::{Config, ConfigManager},
        keyboard::TypingSpeed,
        app_logic::{MenuStructure, MenuItem, create_menu_structure, MenuAction, handle_menu_event},
    };
    use tempfile::TempDir;
    
    // Helper function to create test config manager
    fn create_test_config_manager(typing_speed: TypingSpeed, left_click_paste: bool) -> Arc<ConfigManager> {
    ...
    fn test_submenu_structure_conversion() {
        ...
        let checked_items: Vec<_> = submenu_items.iter().filter_map(|item| {
    ...
    fn test_menu_rebuild_scenarios() {
        ...
        let menu1 = create_menu_structure(initial_config.typing_speed, initial_config.left_click_paste);
        let menu2 = create_menu_structure(config_after_speed.typing_speed, config_after_speed.left_click_paste);
        ...
        assert_ne!(initial_config.left_click_paste, config_after_toggle.left_click_paste);
    }
    ...
    fn test_menu_structure_edge_cases() {
        ...
        for speed in &all_speeds {
            for &left_click in &all_bools {
                ...
                let has_paste = menu.items.iter().any(|item| {
                    matches!(item, MenuItem::Action { id, .. } if id == "paste")
                });
                assert!(has_paste);
                
                // Verify there's always a quit item
                let has_quit = menu.items.iter().any(|item| {
                    matches!(item, MenuItem::Action { id, .. } if id == "quit")
                });
                assert!(has_quit);
            }
        }

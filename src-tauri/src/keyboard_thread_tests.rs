...
mod keyboard_thread_tests {
    use std::sync::Arc;
    use tokio::sync::mpsc;
    use crate::keyboard::{KeyboardEmulator, TypingSpeed, KeyboardCommand};
    ...
    async fn test_keyboard_command_channel_behavior() {
        ...
        tx.send(KeyboardCommand::TypeText("Hello".to_string())).unwrap();
        tx.send(KeyboardCommand::SetSpeed(TypingSpeed::Fast)).unwrap();
        tx.send(KeyboardCommand::TypeText("World".to_string())).unwrap();
    ...
    fn test_channel_closed_behavior() {
        ...
        // Send a command
        tx.send(KeyboardCommand::TypeText("Test".to_string())).unwrap();

...
mod keyboard_execution_tests {
    use crate::keyboard::{KeyboardEmulator, KeyboardCommand, TypingSpeed};
    use tokio::sync::mpsc;
    use std::sync::Arc;
    ...
    fn test_keyboard_thread_lifecycle() {
        ...
        runtime.block_on(async {
            let result = keyboard.type_text("test").await;
            assert!(result.is_ok());
        });
    }
    ...
    fn test_keyboard_command_processing() {
        ...
        let runtime = tokio::runtime::Runtime::new().unwrap();
        runtime.block_on(async {
            tx.send(KeyboardCommand::SetSpeed(TypingSpeed::Fast)).await.unwrap();
            tx.send(KeyboardCommand::TypeText("Hello".to_string())).await.unwrap();
        });
    ...
    fn test_channel_capacity_handling() {
        ...
        runtime.block_on(async {
            // Fill the channel
            tx.send(KeyboardCommand::TypeText("1".to_string())).await.unwrap();
            tx.send(KeyboardCommand::TypeText("2".to_string())).await.unwrap();
            
            // This would block if channel is full, so try_send
            let result = tx.try_send(KeyboardCommand::TypeText("3".to_string()));
            ...
            let result2 = tx.try_send(KeyboardCommand::TypeText("3".to_string()));
